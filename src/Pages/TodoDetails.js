import { Modal, ScrollView, StyleSheet, Text, TouchableOpacity, View, Alert, Image, Pressable } from "react-native"
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
                {
                    storedData.map((value, index) => (
                        <TouchableOpacity key={index} onPress={() => deleteData(index, value)} style={styles.msgBox}>
                            <Text style={styles.todo}>{index + 1}. {value}</Text>
                        </TouchableOpacity>
                    ))
                }
            </ScrollView>
            : <View style={{ flex: 1, alignSelf: 'center', justifyContent: 'center',marginTop:-40 }}>
                <Image source={Nothing} style={{ width: 250, height: 250, resizeMode:'contain' }} />
                <Text style={{fontSize:20,fontWeight:'bold', textAlign:'center',marginTop:15}}>No Schedule!!!</Text>
            </View>}
        </View>
    )
}
